...
public class TestCase3 {
	/*
	 * @Test public void testGetLeadingAnnotation() { String case1 = "@Test@Ignore";
	 * String case2 = "@Test(timeout=10) @Ignore(\"Hello world\")"; String case3 =
	 * "@ Test@ Ignore ( \"\\\"\" )"; String case4 =
	 * "@Test(timeout =    getTimeout())@Ignore"; String case5 =
	 * "@Test public void test"; String case6 = "@Ignore()"; String case7 =
	 * "@Ignore(()"; String case8 = "@Ignore()("; String tmp =
	 * Utils.getLeadingAnnotation(case1); String remaining =
	 * case1.substring(case1.indexOf(tmp) + tmp.length()); assertEquals(tmp,
	 * "@Test"); tmp = Utils.getLeadingAnnotation(remaining); assertEquals(tmp,
	 * "@Ignore"); remaining = remaining.substring(remaining.indexOf(tmp) +
	 * tmp.length()); tmp = Utils.getLeadingAnnotation(remaining); assertNull(tmp);
	 * 
	 * remaining = case2; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertEquals(tmp, "@Test(timeout=10)"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertEquals(tmp,
	 * "@Ignore(\"Hello world\")"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertNull(tmp);
	 * 
	 * remaining = case3; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertEquals(tmp, "@ Test"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertEquals(tmp,
	 * "@ Ignore ( \"\\\"\" )"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertNull(tmp);
	 * 
	 * remaining = case4; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertEquals(tmp, "@Test(timeout =    getTimeout())"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertEquals(tmp, "@Ignore");
	 * remaining = remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertNull(tmp);
	 * 
	 * remaining = case5; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertEquals(tmp, "@Test "); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertNull(tmp);
	 * 
	 * remaining = case6; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertEquals(tmp, "@Ignore()"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertNull(tmp);
	 * 
	 * remaining = case7; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertNull(tmp);
	 * 
	 * remaining = case8; tmp = Utils.getLeadingAnnotation(remaining);
	 * assertEquals(tmp, "@Ignore()"); remaining =
	 * remaining.substring(remaining.indexOf(tmp) + tmp.length()); tmp =
	 * Utils.getLeadingAnnotation(remaining); assertNull(tmp); }
	 */
/*	
	...
	@Test
	@Ignore public void testStripBlockComment() {
		String case1 = "	 * ABC\r\n"
				+ "	 *  DEF\r\n"
				+ "	 *   GHI";
